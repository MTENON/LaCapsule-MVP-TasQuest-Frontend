...
function ButtonEmpty({ icon, variant, children }) {
    const buttonStyles = {
        primary: {
            backgroundColor: "transparent",
            border: "5px solid #fcd757",
        },
        secondary: {
            backgroundColor: "transparent",
            border: "5px solid #a50104",
        },
    };
    ...
    return (
        <button
            onClick={handleClick}
            className={styles.basiqueStyleEmpty}
            style={buttonStyles[variant]}
        >
            <Icon className={styles.iconSize} icon={icon} /> {children}
        </button>
    );
